...
//
static void readKRtFileMiddlebury_ori (string filename, vector<Camera> cameras, InputFiles inputFiles)
{
    ifstream myfile;
    myfile.open( filename, ifstream::in );
    string line;
...
static void read_in_ex (string filename, Camera cam, InputFiles inputFiles)
{
    ifstream myfile;
    myfile.open( filename, ifstream::in );
    string line;
    ...
    stringstream ss(line);
    // string tmp;
    ss >> R(0,0) >> R(0,1) >> R(0,2) >> vt(0);
    
    getline (myfile, line); 
    ss.str(line);
    ss >> R(1,0) >> R(1,1) >> R(1,2) >> vt(1);

    getline (myfile, line);
    ss.str(line);
    ss >> R(2,0) >> R(2,1) >> R(2,2) >> vt(2); //
    ...
    double temp; //used for store temp K values
    ss >> temp; // K(0,0)
    K(0,0) = temp;// / 2.0;
    ss >> temp; // K(0,1)
    K(0,1) = temp;// / 2.0;
...
static int readDmbNormal (const char *filename, Mat_<Vec3f> &img, float * data)
{
    FILE *inimage;
    ...
    // float* data;
    data = (float*) malloc (sizeof(float)*dataSize); //0727
...
// read ground truth depth map file (dmb) (provided by Tola et al. "DAISY: A Fast Local Descriptor for Dense Matching" http://cvlab.epfl.ch/software/daisy)
static int readDmb(const char *filename, Mat_<float> &img, float * data)
{
    FILE *inimage;
    inimage = fopen(filename, "rb");
    if (!inimage){
